...
const outputDir = path.join(frontendRoot, 'reports/stadium/daegu-operator-reference-p31-sky-first-base-source-apply');
const approvalWriteJsonPath = path.join(outputDir, 'daegu-operator-reference-p31-sky-first-base-approval-write.json');
const approvalWriteCsvPath = path.join(outputDir, 'daegu-operator-reference-p31-sky-first-base-approval-write.csv');
const approvalWriteMdPath = path.join(outputDir, 'daegu-operator-reference-p31-sky-first-base-approval-write.md');
const postwriteJsonPath = path.join(outputDir, 'daegu-operator-reference-p31-sky-first-base-postwrite.json');
const postwriteCsvPath = path.join(outputDir, 'daegu-operator-reference-p31-sky-first-base-postwrite.csv');
const postwriteMdPath = path.join(outputDir, 'daegu-operator-reference-p31-sky-first-base-postwrite.md');
...
const approvalReviewedAt = '2026-05-25T01:25:00+09:00';
const traceVersion = 'DAEGU_OPERATOR_REFERENCE_P31_SKY_FIRST_BASE_APPROVED_V1';
const expectedMapVersion = 'DAEGU_SAMSUNG_LIONS_PARK_2025_OPERATOR_REFERENCE_P31_SKY_FIRST_BASE_APPROVED_V1';
const imageWidth = 4096;
const imageHeight = 4096;

const targetRows = [
  {
    draftId: 'RAPAK_REF_155',
    id: 'daegu-sky-lower-s4',
    block: 'S-4',
    name: 'SKY 하단 지정석 S-4',
  },
  {
    draftId: 'RAPAK_REF_160',
    id: 'daegu-sky-lower-s5',
    block: 'S-5',
    name: 'SKY 하단 지정석 S-5',
  },
  {
    draftId: 'RAPAK_REF_164',
    id: 'daegu-sky-lower-s6',
    block: 'S-6',
    name: 'SKY 하단 지정석 S-6',
  },
  {
    draftId: 'RAPAK_REF_168',
    id: 'daegu-sky-lower-s7',
    block: 'S-7',
    name: 'SKY 하단 지정석 S-7',
  },
  {
    draftId: 'RAPAK_REF_172',
    id: 'daegu-sky-lower-s8',
    block: 'S-8',
    name: 'SKY 하단 지정석 S-8',
  },
  {
    draftId: 'RAPAK_REF_176',
    id: 'daegu-sky-lower-s9',
    block: 'S-9',
    name: 'SKY 하단 지정석 S-9',
  },
  {
    draftId: 'RAPAK_REF_186',
    id: 'daegu-sky-lower-s10',
    block: 'S-10',
    name: 'SKY 하단 지정석 S-10',
  },
];

const targetDraftIds = new Set(targetRows.map((row) => row.draftId));
const expectedSelectableRows = 121;
...
function buildApprovalChecks(template, candidate) {
  ...
  addCheck('P21_CURRENT_HAS_FIVE_APPROVED_OR_TARGETS_ALREADY_APPROVED', counts.approvedRows === 5 || (counts.approvedRows === 12 && targetAlreadyApproved), `CURRENT_APPROVED_ROWS_${counts.approvedRows}`);
  addCheck('P21_APPROVED_ROWS_TWELVE', candidateCounts.approvedRows === 12, `PROJECTED_APPROVED_ROWS_${candidateCounts.approvedRows}`);
  addCheck('P21_PENDING_ROWS_TEN', candidateCounts.pendingRows === 10, `PROJECTED_PENDING_ROWS_${candidateCounts.pendingRows}`);
  addCheck('TARGET_SEVEN_ROWS_PRESENT', targetCurrentRows.length === 7 && targetCandidateRows.length === 7, `TARGET_ROWS_${targetCurrentRows.length}`);
  addCheck('TARGET_SEVEN_ROWS_APPROVED', targetCandidateRows.every(targetIsApproved), 'P31_TARGET_ROWS_MUST_BE_APPROVED');
  ...
  }), 'P31_TARGET_GEOMETRY_CHANGED');
  addCheck('TARGET_SEVEN_SOURCE_KEYS_NOT_UNRELATED_ACTIVE', duplicateSourceTargets.length === 0 || targetAlreadyApproved || sourceTargetsAlreadyApplied, `P31_DUPLICATE_SOURCE_TARGET_${duplicateSourceTargets.map((row) => row.draftId).join('|')}`);
  addCheck('SOURCE_WRITE_BLOCKED_DURING_APPROVAL_WRITE', true, 'P31_APPROVAL_WRITE_MUST_NOT_TOUCH_SOURCE');
...
async function writeApproval() {
  ...
  const payload = {
    ...
    policy: {
      note: 'P31-A approves seven SKY S-4~S-10 P21 rows. P31-A source write applies RAPAK_REF_155, RAPAK_REF_160, RAPAK_REF_164, RAPAK_REF_168, RAPAK_REF_172, RAPAK_REF_176, RAPAK_REF_186 only. P31-A keeps the remaining 10 SKY S-11~S-20 rows pending.',
      sourceDataWritePerformed: false,
      productionWriteAllowed: false,
    },
    summary,
  ...
  await fs.writeFile(approvalWriteMdPath, [
    '# 대구 operator reference P31-A SKY first-base approval write',
    '',
    `- status: \`${summary.status}\``,
...
function buildPostwriteChecks(template) {
  ...
  addCheck('P21_APPROVED_ROWS_TWELVE', counts.approvedRows === 12, `APPROVED_ROWS_${counts.approvedRows}`);
  addCheck('P21_PENDING_ROWS_TEN', counts.pendingRows === 10, `PENDING_ROWS_${counts.pendingRows}`);
  addCheck('TARGET_SEVEN_ROWS_APPROVED', targetTemplateRows.length === 7 && targetTemplateRows.every(targetIsApproved), 'P31_TARGET_APPROVAL_MISSING');
  addCheck('APPROVED_SEVEN_BLOCKS_SOURCE_APPLIED', sourceBlocks.every((row) => Boolean(row.block)), `MISSING_SOURCE_BLOCKS_${sourceBlocks.filter((row) => !row.block).map((row) => row.target.draftId).join('|')}`);
  addCheck('CURRENT_SELECTABLE_ROWS_121', selectableRows === expectedSelectableRows, `CURRENT_SELECTABLE_ROWS_${selectableRows}`);
  addCheck('TARGET_MAP_VERSION_P31', DAEGU_OPERATOR_REFERENCE_RAPAK_2025_MAP_VERSION === expectedMapVersion, `MAP_VERSION_${DAEGU_OPERATOR_REFERENCE_RAPAK_2025_MAP_VERSION}`);
  addCheck('PENDING_10_ROWS_NOT_APPLIED', pendingRowsInSource.length === 0, `PENDING_ROWS_IN_SOURCE_${pendingRowsInSource.map((row) => row.draftId).join('|')}`);
  ...
  sourceBlocks.forEach(({ target, block, templateRow }) => {
    ...
    addCheck(`TARGET_TRACE_VERSION_P31_${target.draftId}`, block?.imageGeometry.geometryVersion === traceVersion && block?.imageGeometry.traceVersion === traceVersion, `TRACE_VERSION_${block?.imageGeometry.geometryVersion ?? 'MISSING'}`);
...
async function writePostwrite() {
  ...
  const summary = {
    status: failedChecks.length === 0 ? 'p31-sky-first-base-postwrite-applied' : 'p31-sky-first-base-postwrite-blocked',
    targetDraftIds: Array.from(targetDraftIds),
  ...
  if (requireApplied && summary.status !== 'p31-sky-first-base-postwrite-applied') {
    throw new Error(`P31-A SKY first-base postwrite blocked: failedChecks=${failedChecks.length}`);
  }
  ...
  const payload = {
    ...
    policy: {
      note: 'P31-A source write applies RAPAK_REF_155, RAPAK_REF_160, RAPAK_REF_164, RAPAK_REF_168, RAPAK_REF_172, RAPAK_REF_176, RAPAK_REF_186 only. P31-A keeps the remaining 10 SKY S-11~S-20 rows pending.',
      productionWriteAllowed: false,
  ...
  await fs.writeFile(postwriteMdPath, [
    '# 대구 operator reference P31-A SKY first-base postwrite',
    '',
    `- status: \`${summary.status}\``,
    `- approved rows: \`${summary.approvedRows}\``,
    `- pending rows: \`${summary.pendingRows}\``,
