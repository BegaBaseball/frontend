...
function buildMarkdown(report) {
  return [
    ...
    `- missing expected files: \`${report.expectedIncludedInventory.missing.length}\``,
...
async function main() {
  ...
  const blockers = [
    ...
    ...expectedIncludedInventory.missing.map((file) => ({
      file,
      reason: 'Expected standard shell PR file is not present in dirty inventory.',
    })),
