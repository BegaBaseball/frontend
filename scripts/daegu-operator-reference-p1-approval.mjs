...
const outputDir = path.join(frontendRoot, 'reports/stadium/daegu-operator-reference-p1-approval');
...
const packetJsonPath = path.join(outputDir, 'daegu-operator-reference-p1-approval-packet.json');
const packetCsvPath = path.join(outputDir, 'daegu-operator-reference-p1-approval-packet.csv');
const packetMdPath = path.join(outputDir, 'daegu-operator-reference-p1-approval-packet.md');
const operatorInputJsonPath = path.join(operatorInputDir, 'daegu-operator-reference-p1-operator-input.json');
const operatorInputCsvPath = path.join(operatorInputDir, 'daegu-operator-reference-p1-operator-input.csv');
const gateJsonPath = path.join(gateDir, 'daegu-operator-reference-p1-approval-gate.json');
const gateCsvPath = path.join(gateDir, 'daegu-operator-reference-p1-approval-gate.csv');
const gateMdPath = path.join(gateDir, 'daegu-operator-reference-p1-approval-gate.md');
const dryRunPlanPath = path.join(gateDir, 'daegu-operator-reference-p1-dry-run-apply-plan.json');
...
const geometryVersion = 'DAEGU_OPERATOR_REFERENCE_P1_APPROVED_DRY_RUN_V1';
const expectedP1VisibleLabels = [
  'TR1',
  'TR2',
  'TR3',
  'TR4',
...
const targetLabelSet = new Set(expectedP1VisibleLabels);
...
async function buildPacket() {
  ...
  const p1Rows = exactRows
    ...
    .map((candidate) => {
      ...
      return {
        ...
        riskFlags: [
          ...
          'P1_OPERATOR_REFERENCE_EXACT_LABEL_MATCH',
    ...
    .sort((a, b) => expectedP1VisibleLabels.indexOf(a.visibleLabel) - expectedP1VisibleLabels.indexOf(b.visibleLabel));

  const foundLabels = new Set(p1Rows.map((row) => row.visibleLabel));
  const missingExpectedLabels = expectedP1VisibleLabels.filter((label) => !foundLabels.has(label));
  if (missingExpectedLabels.length > 0) {
    throw new Error(`Expected P1 labels missing from inventory: ${missingExpectedLabels.join(', ')}`);
  }

  await fs.mkdir(cropDir, { recursive: true });
  const candidates = [];
  for (const candidate of p1Rows) {
    candidates.push({
      ...candidate,
  ...
  const packet = {
    status: 'p1-approval-packet-ready',
    ...
    policy: {
      ...
      note: 'This packet creates 4096 operator-reference P1 review evidence only. It never writes src/data/daeguSeatData.ts.',
    },
    summary: {
      candidateCount: candidates.length,
      targetLabels: expectedP1VisibleLabels,
  ...
  await fs.writeFile(packetMdPath, [
    '# 대구 operator reference P1 approval packet',
    '',
  ...
  console.log(`p1_approval_packet_markdown:${packetMdPath}`);
  console.log(`p1_approval_packet_json:${packetJsonPath}`);
  console.log(`p1_operator_input_json:${operatorInputJsonPath}`);
  console.log(`p1_crop_dir:${cropDir}`);
...
async function runGate() {
  ...
  const gate = {
    ...
    policy: {
      ...
      dryRunOnly: true,
      requireApproved,
      note: 'This gate validates P1 operator input and emits a dry-run plan only. It never writes src/data/daeguSeatData.ts.',
    },
    summary: {
      ...
      readyForDryRunReview: status === 'p1-approval-gate-dry-run-ready',
    },
    validations,
  };
  const dryRunPlan = {
    status: dryRunRows.length > 0 ? 'p1-dry-run-plan-ready' : 'p1-dry-run-plan-empty',
  ...
  await fs.writeFile(gateMdPath, [
    '# 대구 operator reference P1 approval gate',
    '',
    `- status: \`${gate.status}\``,
  ...
  console.log(`p1_approval_gate_markdown:${gateMdPath}`);
  console.log(`p1_approval_gate_json:${gateJsonPath}`);
  console.log(`p1_dry_run_plan_json:${dryRunPlanPath}`);
