...
const outputDir = path.join(frontendRoot, 'reports/stadium/daegu-operator-reference-p31-sky-third-base-source-apply');
const approvalWriteJsonPath = path.join(outputDir, 'daegu-operator-reference-p31-sky-third-base-approval-write.json');
const approvalWriteCsvPath = path.join(outputDir, 'daegu-operator-reference-p31-sky-third-base-approval-write.csv');
const approvalWriteMdPath = path.join(outputDir, 'daegu-operator-reference-p31-sky-third-base-approval-write.md');
const postwriteJsonPath = path.join(outputDir, 'daegu-operator-reference-p31-sky-third-base-postwrite.json');
const postwriteCsvPath = path.join(outputDir, 'daegu-operator-reference-p31-sky-third-base-postwrite.csv');
const postwriteMdPath = path.join(outputDir, 'daegu-operator-reference-p31-sky-third-base-postwrite.md');
...
const approvalReviewedAt = '2026-05-25T02:20:00+09:00';
const traceVersion = 'DAEGU_OPERATOR_REFERENCE_P31_SKY_THIRD_BASE_APPROVED_V1';
const expectedMapVersion = 'DAEGU_SAMSUNG_LIONS_PARK_2025_OPERATOR_REFERENCE_P31_SKY_THIRD_BASE_APPROVED_V1';
const imageWidth = 4096;
const imageHeight = 4096;

const targetRows = [
  { draftId: 'RAPAK_REF_171', id: 'daegu-sky-lower-s16', block: 'S-16', name: 'SKY 하단 지정석 S-16' },
  { draftId: 'RAPAK_REF_167', id: 'daegu-sky-lower-s17', block: 'S-17', name: 'SKY 하단 지정석 S-17' },
  { draftId: 'RAPAK_REF_163', id: 'daegu-sky-lower-s18', block: 'S-18', name: 'SKY 하단 지정석 S-18' },
  { draftId: 'RAPAK_REF_159', id: 'daegu-sky-lower-s19', block: 'S-19', name: 'SKY 하단 지정석 S-19' },
  { draftId: 'RAPAK_REF_156', id: 'daegu-sky-lower-s20', block: 'S-20', name: 'SKY 하단 지정석 S-20' },
];

const targetDraftIds = new Set(targetRows.map((row) => row.draftId));
const expectedSelectableRows = 131;
...
function buildApprovalChecks(template, candidate) {
  ...
  addCheck('P21_CURRENT_HAS_SEVENTEEN_APPROVED_OR_TARGETS_ALREADY_APPROVED', counts.approvedRows === 17 || (counts.approvedRows === 22 && targetAlreadyApproved), `CURRENT_APPROVED_ROWS_${counts.approvedRows}`);
  addCheck('P21_APPROVED_ROWS_TWENTY_TWO', candidateCounts.approvedRows === 22, `PROJECTED_APPROVED_ROWS_${candidateCounts.approvedRows}`);
  addCheck('P21_PENDING_ROWS_ZERO', candidateCounts.pendingRows === 0, `PROJECTED_PENDING_ROWS_${candidateCounts.pendingRows}`);
  ...
  addCheck('TARGET_FIVE_ROWS_APPROVED', targetCandidateRows.every(targetIsApproved), 'P31C_TARGET_ROWS_MUST_BE_APPROVED');
  ...
  }), 'P31C_TARGET_GEOMETRY_CHANGED');
  addCheck('TARGET_FIVE_SOURCE_KEYS_NOT_UNRELATED_ACTIVE', duplicateSourceTargets.length === 0 || targetAlreadyApproved || sourceTargetsAlreadyApplied, `P31C_DUPLICATE_SOURCE_TARGET_${duplicateSourceTargets.map((row) => row.draftId).join('|')}`);
  addCheck('SOURCE_WRITE_BLOCKED_DURING_APPROVAL_WRITE', true, 'P31C_APPROVAL_WRITE_MUST_NOT_TOUCH_SOURCE');
...
async function writeApproval() {
  ...
  const payload = {
    ...
    policy: {
      note: 'P31-C approves five SKY S-16~S-20 P21 rows. P31-C source write applies RAPAK_REF_171, RAPAK_REF_167, RAPAK_REF_163, RAPAK_REF_159, RAPAK_REF_156 only. P31-C clears all remaining SKY S-16~S-20 pending rows.',
      sourceDataWritePerformed: false,
      productionWriteAllowed: false,
    },
    summary,
  ...
  await fs.writeFile(approvalWriteMdPath, [
    '# 대구 operator reference P31-C SKY third-base approval write',
    '',
    `- status: \`${summary.status}\``,
...
function buildPostwriteChecks(template) {
  ...
  addCheck('P21_APPROVED_ROWS_TWENTY_TWO', counts.approvedRows === 22, `APPROVED_ROWS_${counts.approvedRows}`);
  addCheck('P21_PENDING_ROWS_ZERO', counts.pendingRows === 0, `PENDING_ROWS_${counts.pendingRows}`);
  addCheck('TARGET_FIVE_ROWS_APPROVED', targetTemplateRows.length === 5 && targetTemplateRows.every(targetIsApproved), 'P31C_TARGET_APPROVAL_MISSING');
  ...
  addCheck('CURRENT_SELECTABLE_ROWS_131', selectableRows === expectedSelectableRows, `CURRENT_SELECTABLE_ROWS_${selectableRows}`);
  addCheck('TARGET_MAP_VERSION_P31C', DAEGU_OPERATOR_REFERENCE_RAPAK_2025_MAP_VERSION === expectedMapVersion, `MAP_VERSION_${DAEGU_OPERATOR_REFERENCE_RAPAK_2025_MAP_VERSION}`);
  addCheck('NO_PENDING_ROWS_LEFT', pendingRows.length === 0 && pendingRowsInSource.length === 0, `PENDING_ROWS_${pendingRows.map((row) => row.draftId).join('|')}_IN_SOURCE_${pendingRowsInSource.map((row) => row.draftId).join('|')}`);
  ...
  sourceBlocks.forEach(({ target, block, templateRow }) => {
    ...
    addCheck(`TARGET_TRACE_VERSION_P31C_${target.draftId}`, block?.imageGeometry.geometryVersion === traceVersion && block?.imageGeometry.traceVersion === traceVersion, `TRACE_VERSION_${block?.imageGeometry.geometryVersion ?? 'MISSING'}`);
...
async function writePostwrite() {
  ...
  const summary = {
    status: failedChecks.length === 0 ? 'p31-sky-third-base-postwrite-applied' : 'p31-sky-third-base-postwrite-blocked',
    targetDraftIds: Array.from(targetDraftIds),
  ...
  if (requireApplied && summary.status !== 'p31-sky-third-base-postwrite-applied') {
    throw new Error(`P31-C SKY third-base postwrite blocked: failedChecks=${failedChecks.length}`);
  }
  ...
  const payload = {
    ...
    policy: {
      note: 'P31-C source write applies RAPAK_REF_171, RAPAK_REF_167, RAPAK_REF_163, RAPAK_REF_159, RAPAK_REF_156 only. P31-C clears all remaining SKY S-16~S-20 pending rows.',
      productionWriteAllowed: false,
  ...
  await fs.writeFile(postwriteMdPath, [
    '# 대구 operator reference P31-C SKY third-base postwrite',
    '',
    `- status: \`${summary.status}\``,
    `- approved rows: \`${summary.approvedRows}\``,
    `- pending rows: \`${summary.pendingRows}\``,
