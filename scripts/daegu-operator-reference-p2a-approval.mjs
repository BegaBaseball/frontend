...
const outputDir = path.join(frontendRoot, 'reports/stadium/daegu-operator-reference-p2a-approval');
...
const packetJsonPath = path.join(outputDir, 'daegu-operator-reference-p2a-approval-packet.json');
const packetCsvPath = path.join(outputDir, 'daegu-operator-reference-p2a-approval-packet.csv');
const packetMdPath = path.join(outputDir, 'daegu-operator-reference-p2a-approval-packet.md');
const operatorInputJsonPath = path.join(operatorInputDir, 'daegu-operator-reference-p2a-operator-input.json');
const operatorInputCsvPath = path.join(operatorInputDir, 'daegu-operator-reference-p2a-operator-input.csv');
const gateJsonPath = path.join(gateDir, 'daegu-operator-reference-p2a-approval-gate.json');
const gateCsvPath = path.join(gateDir, 'daegu-operator-reference-p2a-approval-gate.csv');
const gateMdPath = path.join(gateDir, 'daegu-operator-reference-p2a-approval-gate.md');
const dryRunPlanPath = path.join(gateDir, 'daegu-operator-reference-p2a-dry-run-apply-plan.json');
...
const geometryVersion = 'DAEGU_OPERATOR_REFERENCE_P2A_APPROVED_DRY_RUN_V1';
const expectedP2AVisibleLabels = [
  'LF1',
  'LF2',
  'LF3',
  'LF4',
...
const targetLabelSet = new Set(expectedP2AVisibleLabels);
...
async function buildPacket() {
  ...
  const p2aRows = exactRows
    ...
    .map((candidate) => {
      ...
      return {
        ...
        riskFlags: [
          ...
          'P2A_OPERATOR_REFERENCE_EXACT_LABEL_MATCH',
    ...
    .sort((a, b) => expectedP2AVisibleLabels.indexOf(a.visibleLabel) - expectedP2AVisibleLabels.indexOf(b.visibleLabel));

  const foundLabels = new Set(p2aRows.map((row) => row.visibleLabel));
  const missingExpectedLabels = expectedP2AVisibleLabels.filter((label) => !foundLabels.has(label));
  if (missingExpectedLabels.length > 0) {
    throw new Error(`Expected P2A labels missing from inventory: ${missingExpectedLabels.join(', ')}`);
  }

  await fs.mkdir(cropDir, { recursive: true });
  const candidates = [];
  for (const candidate of p2aRows) {
    candidates.push({
      ...candidate,
  ...
  const packet = {
    status: 'p2a-approval-packet-ready',
    ...
    policy: {
      ...
      note: 'This packet creates 4096 operator-reference P2A LF review evidence only. It never writes src/data/daeguSeatData.ts.',
    },
    summary: {
      candidateCount: candidates.length,
      targetLabels: expectedP2AVisibleLabels,
  ...
  await fs.writeFile(packetMdPath, [
    '# 대구 operator reference P2A approval packet',
    '',
  ...
  console.log(`p2a_approval_packet_markdown:${packetMdPath}`);
  console.log(`p2a_approval_packet_json:${packetJsonPath}`);
  console.log(`p2a_operator_input_json:${operatorInputJsonPath}`);
  console.log(`p2a_crop_dir:${cropDir}`);
...
async function runGate() {
  ...
  const gate = {
    ...
    policy: {
      ...
      dryRunOnly: true,
      requireApproved,
      note: 'This gate validates P2A operator input and emits a dry-run plan only. It never writes src/data/daeguSeatData.ts.',
    },
    summary: {
      ...
      readyForDryRunReview: status === 'p2a-approval-gate-dry-run-ready',
    },
    validations,
  };
  const dryRunPlan = {
    status: dryRunRows.length > 0 ? 'p2a-dry-run-plan-ready' : 'p2a-dry-run-plan-empty',
  ...
  await fs.writeFile(gateMdPath, [
    '# 대구 operator reference P2A approval gate',
    '',
    `- status: \`${gate.status}\``,
  ...
  console.log(`p2a_approval_gate_markdown:${gateMdPath}`);
  console.log(`p2a_approval_gate_json:${gateJsonPath}`);
  console.log(`p2a_dry_run_plan_json:${dryRunPlanPath}`);
