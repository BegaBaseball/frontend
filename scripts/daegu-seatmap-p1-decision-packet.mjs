...
const PACKET_VERSION = 'DAEGU_P1_DECISION_PACKET_V1';
...
const inputCsvPath = path.join(p1ReportDir, 'daegu-seatmap-p1-operator-input.csv');
const checklistPath = path.join(p1ReportDir, 'daegu-seatmap-p1-operator-checklist.md');
const readinessPath = path.join(p1ReportDir, 'daegu-seatmap-p1-operator-readiness.md');
...
if (inputRows.length !== EXPECTED.rows) blockers.push(`P1_INPUT_ROW_COUNT_MISMATCH:${inputRows.length}:${EXPECTED.rows}`);
...
const expectedCounts = [
  ['P1_MANUAL_TRACE_REQUIRED_ROWS', actionCounts.OPERATOR_MANUAL_TRACE_REQUIRED ?? 0, EXPECTED.manualTraceRequiredRows],
  ['P1_SHARED_CANDIDATE_BOUNDARY_ROWS', actionCounts.OPERATOR_SEPARATE_SHARED_CANDIDATE_BOUNDARY ?? 0, EXPECTED.sharedCandidateBoundaryRows],
  ['P1_CORRECTED_PATH_REQUIRED_ROWS', actionCounts.OPERATOR_CORRECTED_PATH_REQUIRED ?? 0, EXPECTED.correctedPathRequiredRows],
];
...
const summary = {
  ...
  sharedCandidateBoundaryRows: actionCounts.OPERATOR_SEPARATE_SHARED_CANDIDATE_BOUNDARY ?? 0,
  ...
  blockers,
  nextCommandsAfterP0ClosedAndOperatorInput: [
...
const report = {
  ...
  safetyContract: [
    ...
    'P1 write-template remains blocked until P0 is closed.',
...
const jsonPath = path.join(p1ReportDir, 'daegu-seatmap-p1-decision-packet.json');
const csvPath = path.join(p1ReportDir, 'daegu-seatmap-p1-decision-packet.csv');
const markdownPath = path.join(p1ReportDir, 'daegu-seatmap-p1-decision-packet.md');
...
const markdownRows = rows.flatMap((row) => [
  `## ${row.block}`,
  '',
  `![${row.block}](${path.relative(p1ReportDir, row.evidenceAbsolutePath)})`,
  '',
  markdownTable(
    ['field', 'value'],
    [
...
console.log(`p1_decision_packet_json:${jsonPath}`);
console.log(`p1_decision_packet_csv:${csvPath}`);
console.log(`p1_decision_packet_markdown:${markdownPath}`);
