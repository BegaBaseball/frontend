...
const outputDir = path.join(frontendRoot, 'reports/stadium/daegu-operator-reference-p2b-approval');
...
const packetJsonPath = path.join(outputDir, 'daegu-operator-reference-p2b-approval-packet.json');
const packetCsvPath = path.join(outputDir, 'daegu-operator-reference-p2b-approval-packet.csv');
const packetMdPath = path.join(outputDir, 'daegu-operator-reference-p2b-approval-packet.md');
const operatorInputJsonPath = path.join(operatorInputDir, 'daegu-operator-reference-p2b-operator-input.json');
const operatorInputCsvPath = path.join(operatorInputDir, 'daegu-operator-reference-p2b-operator-input.csv');
const gateJsonPath = path.join(gateDir, 'daegu-operator-reference-p2b-approval-gate.json');
const gateCsvPath = path.join(gateDir, 'daegu-operator-reference-p2b-approval-gate.csv');
const gateMdPath = path.join(gateDir, 'daegu-operator-reference-p2b-approval-gate.md');
const dryRunPlanPath = path.join(gateDir, 'daegu-operator-reference-p2b-dry-run-apply-plan.json');
...
const geometryVersion = 'DAEGU_OPERATOR_REFERENCE_P2B_APPROVED_DRY_RUN_V1';
const expectedP2BVisibleLabels = [
  'F1',
  'F2',
  'MR10',
];

const targetLabelSet = new Set(expectedP2BVisibleLabels);
...
async function buildPacket() {
  ...
  const p2bRows = exactRows
    ...
    .map((candidate) => {
      ...
      return {
        ...
        riskFlags: [
          ...
          'P2B_OPERATOR_REFERENCE_EXACT_LABEL_MATCH',
    ...
    .sort((a, b) => expectedP2BVisibleLabels.indexOf(a.visibleLabel) - expectedP2BVisibleLabels.indexOf(b.visibleLabel));

  const foundLabels = new Set(p2bRows.map((row) => row.visibleLabel));
  const missingExpectedLabels = expectedP2BVisibleLabels.filter((label) => !foundLabels.has(label));
  if (missingExpectedLabels.length > 0) {
    throw new Error(`Expected P2B labels missing from inventory: ${missingExpectedLabels.join(', ')}`);
  }

  await fs.mkdir(cropDir, { recursive: true });
  const candidates = [];
  for (const candidate of p2bRows) {
    candidates.push({
      ...candidate,
  ...
  const packet = {
    status: 'p2b-approval-packet-ready',
    ...
    policy: {
      ...
      note: 'This packet creates 4096 operator-reference P2B special outfield review evidence only. It never writes src/data/daeguSeatData.ts.',
    },
    summary: {
      candidateCount: candidates.length,
      targetLabels: expectedP2BVisibleLabels,
  ...
  await fs.writeFile(packetMdPath, [
    '# 대구 operator reference P2B approval packet',
    '',
  ...
  console.log(`p2b_approval_packet_markdown:${packetMdPath}`);
  console.log(`p2b_approval_packet_json:${packetJsonPath}`);
  console.log(`p2b_operator_input_json:${operatorInputJsonPath}`);
  console.log(`p2b_crop_dir:${cropDir}`);
...
async function runGate() {
  ...
  const gate = {
    ...
    policy: {
      ...
      dryRunOnly: true,
      requireApproved,
      note: 'This gate validates P2B operator input and emits a dry-run plan only. It never writes src/data/daeguSeatData.ts.',
    },
    summary: {
      ...
      readyForDryRunReview: status === 'p2b-approval-gate-dry-run-ready',
    },
    validations,
  };
  const dryRunPlan = {
    status: dryRunRows.length > 0 ? 'p2b-dry-run-plan-ready' : 'p2b-dry-run-plan-empty',
  ...
  await fs.writeFile(gateMdPath, [
    '# 대구 operator reference P2B approval gate',
    '',
    `- status: \`${gate.status}\``,
  ...
  console.log(`p2b_approval_gate_markdown:${gateMdPath}`);
  console.log(`p2b_approval_gate_json:${gateJsonPath}`);
  console.log(`p2b_dry_run_plan_json:${dryRunPlanPath}`);
