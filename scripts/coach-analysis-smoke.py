...
USER = {"id": 123, "email": "test@example.com", "name": "TestUser", "handle": "testuser",
        "favoriteTeam": "HH", "role": "ROLE_USER", "hasPassword": True, "profileImageUrl": None}

OLD_STRINGS = [
    "분석 기준 팀 선택", "분석 대상 팀 선택", "분석 집중 항목",
    "AI 코치 경기 리뷰 시작", "리뷰 결과 · 주의 변수",
]

GAME = {"gameId": GAME_ID, "gameDate": "2026-05-27", "homeTeam": "KT", "awayTeam": "NC",
        "stadium": "수원", "startTime": "18:30", "homeScore": 5, "awayScore": 8,
        "winner": "NC", "gameStatus": "FINAL", "seasonId": 2026, "leagueType": "REGULAR",
        "homePitcher": {"name": "배제성"}, "awayPitcher": {"name": "라일리"}}
GAME_DETAIL = {"gameId": GAME_ID, "gameDate": "2026-05-27", "homeTeam": "KT", "awayTeam": "NC",
               "stadiumName": "수원", "startTime": "18:30", "homeScore": 5, "awayScore": 8,
               "homePitcher": "배제성", "awayPitcher": "라일리", "gameStatus": "FINAL",
               "inningScores": [], "summary": []}

FULL_ANALYSIS = {
    "summary": "", "verdict": "KT는 **선발 조기 강판**이 패인.",
    "strengths": ["NC 불펜 ERA 1.80"], "weaknesses": ["KT 선발 ERA 5.40"],
    "risks": [
    ...
    "why_it_matters": ["선발 ERA 격차가 초반 흐름 결정", "불펜 깊이가 후반 우위로 직결"],
    "swing_factors": ["7회초 역전 2점 홈런"], "watch_points": ["KT 마무리 등판 시점"],
...
def build_structured(analysis):
    return {
        ...
        "key_metrics": [{"label": "최종 스코어", "value": "NC 8 / KT 5",
                         "status": "danger", "trend": "down", "is_critical": True}],
        "analysis": analysis,
...
def make_sse(*, analysis=FULL_ANALYSIS, win_prob=0.38, completed=True,
             data_quality="grounded", manual=False) -> str:
    if manual:
        meta = {
            ...
            "manual_data_request": {"missing_items": [
                {"key": "game_id", "label": "경기 ID", "reason": "경기 row 부재",
    ...
    meta = {
        ...
        "data_quality": data_quality, "verified": True, "supported_fact_count": 44,
...
# ── scenarios ────────────────────────────────────────────────
def scn_full():        return make_sse()
def scn_full_preview(): return make_sse(completed=False)
def scn_insights0():   return make_sse(analysis={**FULL_ANALYSIS, "why_it_matters": [], "swing_factors": [],
                                                  "watch_points": [], "uncertainty": [], "strengths": [], "weaknesses": []})
def scn_risks0():      return make_sse(analysis={**FULL_ANALYSIS, "risks": []})
def scn_winprob_null():return make_sse(win_prob=None)
def scn_manual():      return make_sse(manual=True)


# scenario, sse, viewport(w), dark, checks
MATRIX = [
    ("full · 1280 · light · review", scn_full(), 1280, False,
     dict(insights=True, risks=True, review="결과를 가른 이유", winprob_dashes=False, body=True)),
    ("full · 390 · light", scn_full(), 390, False,
     dict(insights=True, risks=True, body=True)),
    ("full · 1280 · dark", scn_full(), 1280, True,
     dict(insights=True, risks=True, body=True)),
    ("full · 1280 · light · preview", scn_full_preview(), 1280, False,
     dict(insights=True, risks=True, preview="왜 중요한가", body=True)),
    ("insights=0 · 1280 · light", scn_insights0(), 1280, False,
     dict(insights=False, risks=True, body=True)),
    ("risks=0 · 1280 · light", scn_risks0(), 1280, False,
     dict(insights=True, risks=False, body=True)),
    ("winprob=null · 1280 · light", scn_winprob_null(), 1280, False,
     dict(insights=True, risks=True, body=True)),
    ("manual_data_required · 1280 · light", scn_manual(), 1280, False,
...
VERSUS_TOKEN = "에 불리"
EVIDENCE_CHIP = "실데이터 근거"   # 근거 투명성 칩(헤드라인) — 실데이터 기반 신호
...
def run_case(browser, base_url, label, sse, width, dark, checks):
    auth_state = json.dumps({"state": {"user": USER, "isLoggedIn": True, "isAdmin": False,
                                       "isAuthLoading": False}, "version": 0})
    auth_meta = json.dumps({"version": 1, "lastSuccessAt": int(time.time() * 1000), "lastFailureAt": None})
    init = (
    ...
    if dark:
        ctx.add_init_script("try{document.documentElement.classList.add('dark')}catch(e){}")
    page = ctx.new_page()
    ...
    def handler(route):
        u = route.request.url
        if "/ai/coach/analyze" in u:
            route.fulfill(status=200, headers={"content-type": "text/event-stream"}, body=sse)
        elif f"/api/matches/{GAME_ID}" in u and "day" not in u and "bounds" not in u:
            route.fulfill(status=200, content_type="application/json", body=json.dumps(GAME_DETAIL))
        elif "/api/matches/day" in u:
            route.fulfill(status=200, content_type="application/json",
                          body=json.dumps({"date": "2026-05-27", "games": [GAME]}))
        elif "/api/matches/bounds" in u:
            route.fulfill(status=200, content_type="application/json",
                          body=json.dumps({"hasData": True, "earliestGameDate": "2026-01-01",
                                           "latestGameDate": "2026-05-28"}))
        elif f"/api/predictions/status/{GAME_ID}" in u:
            route.fulfill(status=200, content_type="application/json",
                          body=json.dumps({"gameId": GAME_ID, "homeVotes": 42, "awayVotes": 58,
                                           "userVote": None, "totalVotes": 100}))
        elif "/api/auth/mypage" in u:
            route.fulfill(status=200, content_type="application/json",
                          body=json.dumps({"success": True, "data": {**USER}}))
        elif "/api/auth/reissue" in u:
            route.fulfill(status=200, content_type="application/json",
                          body=json.dumps({"success": True, "data": {"accessToken": FAKE_TOKEN}}))
        elif "/api/chat/my/unread-counts" in u:
            route.fulfill(status=200, content_type="application/json",
                          body=json.dumps({"success": True, "data": 0}))
        elif "/api/notifications" in u:
            route.fulfill(status=200, content_type="application/json", body="[]")
        elif "/api/home" in u:
            route.fulfill(status=200, content_type="application/json", body="{}")
        elif "/api/kbo/rankings" in u or "/api/rankings" in u:
            route.fulfill(status=200, content_type="application/json",
                          body=json.dumps([
                              {"teamId": "KT", "teamName": "KT 위즈", "rank": 5,
                               "wins": 50, "losses": 45, "draws": 2, "winRate": "0.526",
                               "games": 97, "gamesBehind": 3.0},
                              {"teamId": "NC", "teamName": "NC 다이노스", "rank": 3,
                               "wins": 55, "losses": 42, "draws": 1, "winRate": "0.567",
                               "games": 98, "gamesBehind": 1.0},
                          ]))
        else:
            route.continue_()

    page.route(f"{base_url}/api/**", handler)
    page.route(f"{base_url}/ai/**", handler)

    failures = []
    try:
        page.goto(f"{base_url}/prediction?date=2026-05-27", wait_until="domcontentloaded", timeout=25000)
        page.wait_for_timeout(2500)
        if dark:
            page.evaluate("document.documentElement.classList.add('dark')")
        page.locator('[data-testid="prediction-match-enter-detail-btn"]').first.click()
        page.wait_for_timeout(4500)
        page.locator('[data-testid="coach-analysis-open"]').first.click()
        page.wait_for_timeout(8000)  # 자동 실행 대기
        ...
        has_body = page.locator("[role='article']").count() > 0 and page.locator("[role='article']").first.is_visible()
        ...
        if checks.get("notice"):
            if page.locator("[data-testid='coach-analysis-data-quality-note']").count() == 0:
