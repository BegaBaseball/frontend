...
const noticeCardBaseClass = 'w-full max-w-[22rem] gap-2 p-3 pointer-events-auto animate-roll-in-down motion-reduce:animate-none';
...
export default function PredictionTopNotice({
    ...
    return (
      <Card className={`${noticeCardBaseClass} border border-emerald-200 bg-emerald-50 text-emerald-900 dark:border-emerald-700/50 dark:bg-emerald-950/40 dark:text-emerald-100`}>
        <div className="flex flex-wrap items-center gap-3">
          ...
          <p className="text-body font-bold">{runProgressMessage}</p>
  ...
  if (kind === 'FUTURE') {
    if (futureRangeLoadState === 'loading') {
      return (
        <Card className={`${noticeCardBaseClass} border border-sky-200 bg-sky-50 text-sky-900 dark:border-sky-700/40 dark:bg-sky-900/30 dark:text-sky-100`}>
          <div className="inline-flex items-center gap-2 text-body font-bold">
    ...
    return (
      <Card className={`${noticeCardBaseClass} border border-rose-200 bg-rose-50 text-rose-900 dark:border-rose-700/40 dark:bg-rose-900/30 dark:text-rose-100`}>
        <p className="mb-2 text-body font-bold">
  ...
  if (kind === 'ERROR') {
    if (voteStatusError) {
      return (
        <Card className={`${noticeCardBaseClass} border border-rose-200 bg-rose-50 text-rose-900 dark:border-rose-700/40 dark:bg-rose-900/30 dark:text-rose-100`}>
          <p className="mb-2 text-body font-bold">투표 집계 조회 실패: {voteStatusError}</p>
          <div className="flex flex-col gap-2 sm:flex-row">
            <Button
              size="sm"
    ...
    if (deepLinkNotice) {
      return (
        <Card className={`${noticeCardBaseClass} border border-amber-200 bg-amber-50 text-amber-900 dark:border-amber-700/40 dark:bg-amber-900/30 dark:text-amber-100`}>
          <div className="text-body">{deepLinkNotice}</div>
        </Card>
      );
    }
      ...
      return (
        ...
        >
          <div className="mb-2 flex items-center gap-2">
            <span className="inline-flex items-center rounded-full bg-amber-200/80 px-2 py-0.5 text-body font-bold text-amber-900 dark:bg-amber-800/70 dark:text-amber-100">
              부분 결과
            </span>
            <p className="text-body font-bold">투표 집계가 일부만 도착했습니다.</p>
          </div>
          <div className="flex flex-col gap-2 sm:flex-row">
            <Button
              size="sm"
            ...
            <span className="inline-flex items-center text-body text-amber-800/80 dark:text-amber-100/80">
