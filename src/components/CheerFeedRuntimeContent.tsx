        ...
        <>
            {newPostCount > 0 && (
                <button
                    ...
                    className="sticky top-12 z-20 w-full backdrop-blur-sm min-h-11 text-[16px] font-semibold transition-colors flex items-center justify-center gap-2 border-b"
                    style={{
            ...
            <section className="mt-4">
                {isLoading && currentPosts.length === 0 ? (
                    ...
                    <div className="py-8 sm:py-10 px-4 sm:px-6 flex flex-col items-center justify-center gap-4">
                        <div className="flex flex-col items-center gap-3">
                            ...
                            <div className="text-center">
                                ...
                                <p className="mt-1 text-[16px] font-semibold text-slate-500 dark:text-gray-300">
                        ...
                        <button
                            ...
                            className="rounded-full bg-slate-100 dark:bg-secondary px-6 py-2.5 text-[16px] font-semibold text-slate-700 dark:text-gray-200 hover:bg-slate-200 dark:hover:bg-secondary transition-colors"
                        >
                            다시 시도
                        </button>
                    </div>
                    ...
                    <div className="border-b border-border/70 dark:border-border px-4 sm:px-6 py-8 sm:py-10 text-center">
                        <p className="text-[#64748B] font-semibold dark:text-gray-300">로그인이 필요합니다</p>
                        <p className="mt-1 text-[16px] font-semibold text-slate-400 dark:text-gray-300">팔로우한 유저의 글을 보려면 로그인해주세요.</p>
                        <button
                            type="button"
                            onClick={onRequireLogin}
                            className="mt-4 rounded-full px-6 py-2 text-[16px] font-semibold text-white"
                    ...
                    <div className="border-b border-border/70 dark:border-border px-4 sm:px-6 py-8 sm:py-10 text-center">
                        {activeFeedTab === 'following' ? (
                            <>
                                <p className="text-[#64748B] font-semibold dark:text-gray-300">팔로우한 유저가 없습니다</p>
                                <p className="mt-1 text-[16px] font-semibold text-slate-400 dark:text-gray-300">다른 유저를 팔로우하면 여기에 글이 표시됩니다!</p>
                            </>
                        ) : (
                            <>
                                <p className="text-[#64748B] font-semibold dark:text-gray-300">아직 작성된 응원글이 없습니다.</p>
                                <p className="mt-1 text-[16px] font-semibold text-slate-400 dark:text-gray-300">첫 번째 응원글을 남겨보세요!</p>
                            </>
                        )}
                    </div>
                ) : (
                ...
                <div ref={sentinelRef} className="flex min-h-[120px] items-center justify-center">
                    {queryError && currentPosts.length > 0 ? (
                        <div className="flex flex-col items-center gap-2 text-[16px] text-slate-500 dark:text-gray-300">
                            ...
                            <p className="text-[16px] font-semibold text-slate-400 dark:text-slate-300">
                                네트워크 상태를 확인하고 다시 시도해 주세요
                            </p>
                            <button
                                type="button"
                                onClick={() => fetchNextPage()}
                                className="rounded-full border border-slate-200 dark:border-border px-4 py-1.5 text-[16px] font-semibold text-slate-600 dark:text-gray-200 hover:bg-slate-50 dark:hover:bg-secondary"
                            >
                                다시 시도
                            </button>
                        </div>
                        ...
                        <div className="flex items-center gap-2 text-[16px] text-slate-500 dark:text-gray-300">
                            ...
                            <span className="font-semibold">불러오는 중...</span>
                        </div>
                    ) : null}
