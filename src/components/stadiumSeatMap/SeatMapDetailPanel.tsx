...
function EmptyState({ copy }: { copy?: SeatMapCommonCopy }) {
  return (
    <div className="flex h-full flex-col items-center justify-center px-6 py-10 text-center">
      ...
      <p className="mb-1.5 text-sm font-bold text-slate-700 dark:text-slate-200">
  ...
  if (!section) {
    return (
      <div
        ...
        style={{ maxHeight: 'calc(100vh - 32px)', minHeight: 220 }}
      >
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-2.5 px-5 pb-4">
        <InfoTile label={blockLabel} value={adapter.getBlock(section)} />
        <InfoTile label="위치" value={adapter.getSideLabel(section)} />
        <InfoTile label="팬 구분" value={adapter.getFanRoleLabel(section)} />
        <InfoTile label="시야 거리" value={adapter.getDistance?.(section) ?? '-'} />
      </div>
      ...
      {(notes || tags.length > 0) && (
        <div className="border-t border-slate-100 px-5 py-4 dark:border-slate-800">
          {notes && <p className="text-sm font-semibold leading-relaxed text-slate-600 dark:text-slate-300">{notes}</p>}
          {tags.length > 0 && (
