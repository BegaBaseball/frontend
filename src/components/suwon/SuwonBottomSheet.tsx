...
interface BottomSheetProps {
  section: SuwonBlock | null;
  mode: 'light' | 'dark';
  onClose: () => void;
  onUpload: () => void;
}

export default function SuwonBottomSheet({ section, mode, onClose, onUpload }: BottomSheetProps) {
  ...
  const info = SUWON_VIEW_INFO[section.id] ?? SUWON_VIEW_INFO.default;
      ...
      <div className="flex shrink-0 items-center gap-3 px-4 pb-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="flex items-center gap-1.5 text-[11px] text-slate-500">
            {cat.label} · {getSuwonSideLabel(section.side)} · {getSuwonFanRoleLabel(section.fanRole)}
          </div>
        </div>
        <button
          type="button"
      ...
      >
        <div className="mb-4 flex flex-wrap gap-1.5">
          ...
          <span className="rounded-full bg-amber-100 px-2.5 py-1 text-[11px] font-bold text-amber-800">
            {getSuwonSourceLabel(section.sourceConfidence)}
          </span>
        </div>
        ...
        <div className="mb-4 grid grid-cols-2 gap-2">
          ...
          <div className="rounded-xl bg-slate-50 p-2.5 dark:bg-slate-800">
            ...
            <div className="mt-0.5 text-sm font-black text-slate-800 dark:text-white">{getSuwonSideLabel(section.side)}</div>
          </div>
          <div className="rounded-xl bg-slate-50 p-2.5 dark:bg-slate-800">
            <div className="text-[9px] font-bold tracking-widest text-slate-400">팬 구분</div>
            <div className="mt-0.5 text-sm font-black text-slate-800 dark:text-white">{getSuwonFanRoleLabel(section.fanRole)}</div>
          </div>
          <div className="rounded-xl bg-slate-50 p-2.5 dark:bg-slate-800">
