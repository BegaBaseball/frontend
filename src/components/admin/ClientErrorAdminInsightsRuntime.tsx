      ...
      <section className="rounded-2xl border border-slate-800 bg-slate-900/70 p-5">
        <div className="mb-4 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-caption text-slate-400">사용자가 Error ID 기준으로 전송한 최신 제보입니다.</p>
          </div>
        </div>
          ...
          {dashboard?.recentFeedback.length ? dashboard.recentFeedback.map((item) => (
            <div key={`${item.eventId}-${item.occurredAt}`} className="rounded-2xl border border-slate-800 bg-slate-950/60 p-4">
              <div className="mb-2 flex items-center justify-between gap-3">
                ...
                <span className="text-caption text-slate-500">{getTimeAgo(item.occurredAt)}</span>
              </div>
      ...
      <section className="rounded-2xl border border-slate-800 bg-slate-900/70 p-5">
        <div className="mb-4 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-caption text-slate-400">백엔드가 설정된 채널로 전송한 최근 알림 결과입니다.</p>
          </div>
        </div>
          ...
          {dashboard?.recentAlerts.length ? dashboard.recentAlerts.map((item) => (
            <div key={item.id} className="rounded-2xl border border-slate-800 bg-slate-950/60 p-4">
              <div className="mb-2 flex items-center justify-between gap-3">
                ...
                <span className="text-caption text-slate-500">{formatDetailedDateTime(item.notifiedAt)}</span>
              </div>
