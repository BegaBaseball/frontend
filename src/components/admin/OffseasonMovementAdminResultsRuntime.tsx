        ...
        <div className="rounded-2xl border border-sky-500/20 bg-sky-500/5 p-4">
          ...
          {csvReport.errors.length > 0 && (
            <div className="mt-3 rounded-xl border border-slate-800 bg-slate-950/80 p-3">
              <p className="text-sm font-semibold uppercase tracking-[0.18em] text-slate-500">Failed Rows</p>
              <div className="mt-2 space-y-1 text-sm text-slate-300">
                {csvReport.errors.slice(0, 5).map((message) => (
                  <p key={message}>{message}</p>
                ))}
                {csvReport.errors.length > 5 && (
                <p className="text-sm text-slate-500">추가 실패 {csvReport.errors.length - 5}건은 같은 파일을 수정한 뒤 다시 업로드하면 됩니다.</p>
                )}
              </div>
            </div>
          )}
            ...
            <TableBody>
              {filteredMovements.length === 0 ? (
                ...
                filteredMovements.map((movement, index) => (
                  ...
                  >
                    <TableCell className="text-slate-300 text-sm font-semibold">{formatDate(movement.movementDate)}</TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <p className="text-sm font-semibold text-slate-100">{TEAM_DATA[movement.teamCode]?.fullName || movement.teamCode}</p>
                          <p className="text-sm text-slate-500">{movement.teamCode}</p>
                        </div>
                      </div>
                    </TableCell>
                    <TableCell className="text-slate-200 font-semibold">{movement.playerName}</TableCell>
                    <TableCell className="max-w-[280px]">
                      <div className="space-y-1">
                        ...
                        <p className="line-clamp-1 text-sm text-slate-500">{movement.details}</p>
                        )}
                      </div>
                    </TableCell>
                    <TableCell>
