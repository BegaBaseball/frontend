    ...
    if (isError) {
        return (
            <div className="bg-red-50/50 dark:bg-card rounded-2xl p-5 border border-red-100 dark:border-border">
                ...
                <p className="text-body text-[#64748B] dark:text-white">데이터를 불러오지 못했습니다.</p>
                <p className="mt-1 text-body text-slate-500 dark:text-white">
                    네트워크 상태를 확인하고 다시 시도해 주세요
                </p>
                <button
                    type="button"
                    onClick={() => refetch()}
                    className="mt-3 rounded-full border border-red-200 dark:border-border px-4 py-1.5 text-body font-semibold text-red-600 dark:text-red-300 hover:bg-red-100 dark:hover:bg-secondary"
                >
                    다시 시도
                </button>
            </div>
    ...
    if (!hotPosts.length) {
        return (
            <div className="bg-red-50/50 dark:bg-card rounded-2xl p-5 border border-red-100 dark:border-border">
                ...
                <p className="text-body text-[#64748B] dark:text-white">지금은 표시할 인기 글이 없습니다.</p>
            </div>
        );
    }
