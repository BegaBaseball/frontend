...
export default function JamsilBottomSheet({ section, mode, onClose, onUpload }: BottomSheetProps) {
  ...
  const onTouchMove = (e: React.TouchEvent) => {
    ...
    if (dy > 0) setSnap((s) => s === 'full' ? 'half' : 'peek');
    else setSnap((s) => s === 'peek' ? 'half' : 'full');
  ...
  return (
    ...
    >
      <div
        ...
        onClick={() => setSnap((s) => s === 'half' ? 'full' : s === 'full' ? 'peek' : 'half')}
      ...
      >
        ...
        <div className="mb-4 grid grid-cols-2 gap-2">
          ...
          <div className="rounded-xl bg-slate-50 p-2.5 dark:bg-slate-800">
            ...
            <div className="mt-0.5 text-sm font-black text-slate-800 dark:text-white">{getJamsilSideLabel(section.side)}</div>
          </div>
        </div>
