...
import { CHANGWON_CATEGORIES, getChangwonBlockDisplayName, type ChangwonBlock } from '../../data/changwonSeatData';
...
export default function ChangwonUploadFlowModal({ section, mode, onClose, onSubmit }: Props) {
      ...
      >
        <div className="border-b border-slate-100 px-6 pb-4 pt-5 dark:border-slate-800">
          <div className="flex items-start justify-between gap-4">
            <div>
              <div className="mb-1 text-[11px] font-black uppercase tracking-widest text-slate-400">CHANGWON SEAT VIEW</div>
              <h3 className="text-lg font-black tracking-tight text-slate-800 dark:text-white">시야 사진을 추가하세요</h3>
              <p className="mt-1 text-xs text-slate-500">
                <span className="font-bold" style={{ color: accent }}>{getChangwonBlockDisplayName(section)}</span> · {section.seatTypes.join(' · ')}
              </p>
            </div>
            <button
              type="button"
