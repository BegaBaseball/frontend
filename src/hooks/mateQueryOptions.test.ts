...
test('mate query helper는 party mutation 후 관련 query를 일관되게 무효화한다', async () => {
...
test('mate query helper는 host 관리용 my-application prefix invalidate를 지원한다', async () => {
  const { queryClient, invalidateCalls } = createInvalidateRecorder();
...
test('mate query helper는 collection invalidate와 party cache set/remove를 공통화한다', async () => {
...
test('mate cache helper는 승인 후 detail/list/myParties를 직접 갱신한다', () => {
  const queryClient = new QueryClient();
  const party = createParty();
...
test('mate cache helper는 승인 취소 후 myApplication과 myParties를 직접 정리한다', () => {
  const queryClient = new QueryClient();
  const party = createParty({
    id: 21,
    currentParticipants: 2,
  ...
  removeMatePartyFromCollections(queryClient, 21, {
...
test('mate cache helper는 마지막 체크인 후 public list에서 파티를 숨긴다', () => {
  const queryClient = new QueryClient();
  const party = createParty({
    id: 31,
    currentParticipants: 2,
