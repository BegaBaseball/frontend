...
import { INCHEON_BLOCKS } from './incheonSeatData';
import {
  INCHEON_BLOCK_VISIT_GUIDANCE,
  INCHEON_OPERATION_NOTICES,
  INCHEON_OPERATOR_FACILITY_POINTS,
  getIncheonOperatorVisitGuidance,
  selectIncheonActiveOperationNotices,
  type IncheonOperationNotice,
} from './incheonOperatorVisitGuide';

const ISO_DATE_PATTERN = /^\d{4}-\d{2}-\d{2}$/;
const SOURCE_DOCUMENT_ID_PATTERN = /^incheon-operator-\d{8}-[a-z0-9-]+$/;
const FACILITY_POINT_ID_PATTERN = /^incheon-facility-(entrance|concession|restroom|elevator|parking|transit)-[a-z0-9-]+$/;
const OPERATION_NOTICE_ID_PATTERN = /^incheon-operation-notice-\d{8}-[a-z0-9-]+$/;
...
test('인천 운영자 직관 guide는 모든 블록에 fallback 결과를 반환한다', () => {
  INCHEON_BLOCKS.forEach((block) => {
    const guidance = getIncheonOperatorVisitGuidance(block, '2026-05-28');
...
test('인천 운영자 직관 guide는 운영자 자료가 없으면 항목 단위 pending 문구를 유지한다', () => {
  const block = INCHEON_BLOCKS.find((entry) => entry.block === '101B');
  assert.ok(block);
...
test('인천 운영자 직관 데이터는 원본/검수 메타데이터와 ID 규칙을 지킨다', () => {
  const pointIds = new Set<string>();
  const noticeIds = new Set<string>();

  INCHEON_OPERATOR_FACILITY_POINTS.forEach((point) => {
  ...
  INCHEON_BLOCK_VISIT_GUIDANCE.forEach((guidance) => {
  ...
  INCHEON_OPERATION_NOTICES.forEach((notice) => {
...
test('인천 운영자 직관 guide 참조 ID는 실제 facility/notice 데이터만 가리킨다', () => {
  const pointIds = new Set(INCHEON_OPERATOR_FACILITY_POINTS.map((point) => point.id));
  const pointsById = new Map(INCHEON_OPERATOR_FACILITY_POINTS.map((point) => [point.id, point]));
  const blockIds = new Set(INCHEON_BLOCKS.map((block) => block.id));

  INCHEON_BLOCK_VISIT_GUIDANCE.forEach((guidance) => {
    assert.ok(blockIds.has(guidance.blockId), `${guidance.blockId} should map to an Incheon block`);
  ...
  INCHEON_OPERATION_NOTICES.forEach((notice) => {
    notice.affectedBlockIds.forEach((blockId) => {
      assert.ok(blockIds.has(blockId), `${blockId} should map to an Incheon block`);
    });
  });
});

test('인천 날짜별 운영 공지는 KST 날짜와 priority 기준으로 선택된다', () => {
  const notices: IncheonOperationNotice[] = [
    {
      id: 'low-priority',
      ...
      sourceDocumentId: 'incheon-operator-20260528-test-document',
    },
    {
      id: 'high-priority',
      validFrom: '2026-05-28',
  ...
  assert.deepEqual(
    selectIncheonActiveOperationNotices(notices, '2026-05-28').map((notice) => notice.id),
    ['high-priority', 'low-priority'],
  );
  assert.deepEqual(selectIncheonActiveOperationNotices(notices, '2026-05-03'), []);
});

test('인천 운영자 직관 데이터는 외부 URL/자동 수집 계약을 포함하지 않는다', () => {
  const serializedData = JSON.stringify({
    points: INCHEON_OPERATOR_FACILITY_POINTS,
    blockGuidance: INCHEON_BLOCK_VISIT_GUIDANCE,
    notices: INCHEON_OPERATION_NOTICES,
  });
  const source = readFileSync(new URL('./incheonOperatorVisitGuide.ts', import.meta.url), 'utf8');
...
test('인천 운영자 직관 런타임은 원본 PDF/CSV/이미지를 직접 파싱하지 않는다', () => {
  const source = readFileSync(new URL('./incheonOperatorVisitGuide.ts', import.meta.url), 'utf8');

  assert.doesNotMatch(source, /readFile|fetch\(|XMLHttpRequest|\.pdf|\.csv|\.xlsx|\.png|\.webp/i);
});

test('인천 운영자 직관 입력 포맷 문서는 정적 데이터 계약을 고정한다', () => {
  const doc = readFileSync(new URL('../../docs/stadium/incheon-operator-guide-format.md', import.meta.url), 'utf8');

  assert.match(doc, /INCHEON_OPERATOR_FACILITY_POINTS/);
  assert.match(doc, /INCHEON_BLOCK_VISIT_GUIDANCE/);
  assert.match(doc, /INCHEON_OPERATION_NOTICES/);
...
test('인천 운영자 공통 intake 템플릿은 필수 컬럼과 placeholder-only 예시를 유지한다', () => {
  ...
  const incheonRows = rows.filter((row) => row.includes(',INCHEON,'));

  REQUIRED_INTAKE_COLUMNS.forEach((column) => {
    assert.ok(columns.includes(column), `operator intake template should include ${column}`);
  });
  assert.ok(incheonRows.some((row) => row.startsWith('facility,')), 'Incheon template should include a facility placeholder row');
  assert.ok(incheonRows.some((row) => row.startsWith('block,')), 'Incheon template should include a block placeholder row');
  assert.ok(incheonRows.some((row) => row.startsWith('notice,')), 'Incheon template should include a notice placeholder row');
...
test('인천 운영자 공통 정책 문서는 결측과 금지 데이터 계약을 고정한다', () => {
