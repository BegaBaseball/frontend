...
import { SUWON_BLOCKS } from './suwonSeatData';
import {
  SUWON_BLOCK_VISIT_GUIDANCE,
  SUWON_OPERATION_NOTICES,
  SUWON_OPERATOR_FACILITY_POINTS,
  getSuwonOperatorVisitGuidance,
  selectSuwonActiveOperationNotices,
  type SuwonOperationNotice,
} from './suwonOperatorVisitGuide';

const ISO_DATE_PATTERN = /^\d{4}-\d{2}-\d{2}$/;
const SOURCE_DOCUMENT_ID_PATTERN = /^suwon-operator-\d{8}-[a-z0-9-]+$/;
const FACILITY_POINT_ID_PATTERN = /^suwon-facility-(entrance|concession|restroom|elevator|parking|transit)-[a-z0-9-]+$/;
const OPERATION_NOTICE_ID_PATTERN = /^suwon-operation-notice-\d{8}-[a-z0-9-]+$/;
...
test('수원 운영자 직관 guide는 모든 블록에 fallback 결과를 반환한다', () => {
  SUWON_BLOCKS.forEach((block) => {
    const guidance = getSuwonOperatorVisitGuidance(block, '2026-05-28');
...
test('수원 운영자 직관 guide는 운영자 자료가 없으면 항목 단위 pending 문구를 유지한다', () => {
  const block = SUWON_BLOCKS.find((entry) => entry.block === '117');
  assert.ok(block);
...
test('수원 운영자 직관 fallback은 좌석 메타데이터를 출입구/시설/동선으로 대체하지 않는다', () => {
  const block = SUWON_BLOCKS.find((entry) => entry.block === '117');
  assert.ok(block);
...
test('수원 운영자 직관 데이터는 원본/검수 메타데이터와 ID 규칙을 지킨다', () => {
  const pointIds = new Set<string>();
  const noticeIds = new Set<string>();

  SUWON_OPERATOR_FACILITY_POINTS.forEach((point) => {
  ...
  SUWON_BLOCK_VISIT_GUIDANCE.forEach((guidance) => {
  ...
  SUWON_OPERATION_NOTICES.forEach((notice) => {
...
test('수원 운영자 직관 guide 참조 ID는 실제 facility/notice 데이터만 가리킨다', () => {
  const pointIds = new Set(SUWON_OPERATOR_FACILITY_POINTS.map((point) => point.id));
  const pointsById = new Map(SUWON_OPERATOR_FACILITY_POINTS.map((point) => [point.id, point]));
  const blockIds = new Set(SUWON_BLOCKS.map((block) => block.id));

  SUWON_BLOCK_VISIT_GUIDANCE.forEach((guidance) => {
    assert.ok(blockIds.has(guidance.blockId), `${guidance.blockId} should map to a Suwon block`);
  ...
  SUWON_OPERATION_NOTICES.forEach((notice) => {
    notice.affectedBlockIds.forEach((blockId) => {
      assert.ok(blockIds.has(blockId), `${blockId} should map to a Suwon block`);
    });
  });
});

test('수원 날짜별 운영 공지는 KST 날짜와 priority 기준으로 선택된다', () => {
  const notices: SuwonOperationNotice[] = [
    {
      id: 'low-priority',
      ...
      sourceDocumentId: 'suwon-operator-20260528-test-document',
    },
    {
      id: 'high-priority',
      validFrom: '2026-05-28',
  ...
  assert.deepEqual(
    selectSuwonActiveOperationNotices(notices, '2026-05-28').map((notice) => notice.id),
    ['high-priority', 'low-priority'],
  );
  assert.deepEqual(selectSuwonActiveOperationNotices(notices, '2026-05-03'), []);
});

test('수원 운영자 직관 데이터는 외부 URL/자동 수집 계약을 포함하지 않는다', () => {
  const serializedData = JSON.stringify({
    points: SUWON_OPERATOR_FACILITY_POINTS,
    blockGuidance: SUWON_BLOCK_VISIT_GUIDANCE,
    notices: SUWON_OPERATION_NOTICES,
  });
  const source = readFileSync(new URL('./suwonOperatorVisitGuide.ts', import.meta.url), 'utf8');
...
test('수원 운영자 직관 런타임은 원본 PDF/CSV/이미지를 직접 파싱하지 않는다', () => {
  const source = readFileSync(new URL('./suwonOperatorVisitGuide.ts', import.meta.url), 'utf8');

  assert.doesNotMatch(source, /readFile|fetch\(|XMLHttpRequest|\.pdf|\.csv|\.xlsx|\.png|\.webp/i);
});

test('수원 운영자 직관 입력 포맷 문서는 정적 데이터 계약을 고정한다', () => {
  const doc = readFileSync(new URL('../../docs/stadium/suwon-operator-guide-format.md', import.meta.url), 'utf8');

  assert.match(doc, /SUWON_OPERATOR_FACILITY_POINTS/);
  assert.match(doc, /SUWON_BLOCK_VISIT_GUIDANCE/);
  assert.match(doc, /SUWON_OPERATION_NOTICES/);
...
test('수원 운영자 공통 intake 템플릿은 필수 컬럼과 placeholder-only 예시를 유지한다', () => {
  ...
  const suwonRows = rows.filter((row) => row.includes(',SUWON,'));

  REQUIRED_INTAKE_COLUMNS.forEach((column) => {
    assert.ok(columns.includes(column), `operator intake template should include ${column}`);
  });
  assert.ok(suwonRows.some((row) => row.startsWith('facility,')), 'Suwon template should include a facility placeholder row');
  assert.ok(suwonRows.some((row) => row.startsWith('block,')), 'Suwon template should include a block placeholder row');
  assert.ok(suwonRows.some((row) => row.startsWith('notice,')), 'Suwon template should include a notice placeholder row');
